...
--     email:    user@floorplan.studio      password: userfloorplan
--
...
declare
  ...
  admin_email text := 'demo@floorplan.studio';
  admin_pass text := 'DemoPass123!';
...
begin
  ...
  insert into auth.identities (
    ...
    format('{"sub":"%s","email":"%s","email_verified":true,"phone_verified":false}', admin_id::text, admin_email)::jsonb,
    'email', now(), now(), now()
  );
  ...
  values (admin_id, 'admin', 'RoomAI Team', 'RoomAI Team', '')
  on conflict (user_id) do update
    set role = 'admin',
        username = coalesce(public.user_profiles.username, 'RoomAI Team'),
        display_name = coalesce(public.user_profiles.display_name, 'RoomAI Team'),
  ...
  select admin_id, 'Demo Living Room', $DEMO${
    "roomConfig": {
  ...
  where not exists (select 1 from public.projects where user_id = admin_id);
  ...
  insert into auth.users (
    ...
    user_email, crypt(user_pass, gen_salt('bf')),
    now(), now(), now(),
    '', '', '', '',
    '{"provider":"email","providers":["email"]}', '{}'
  )
  ...
  -- 3) Profile row — STANDARD USER (not an admin), public feed identity.
  insert into public.user_profiles (user_id, role, username, display_name, avatar_url)
  values (user_id, 'user', 'RoomAI Demo', 'RoomAI Demo User', '')
  on conflict (user_id) do update
    set role = 'user',
  ...
  -- 4) Sample project so the standard user has something to look at too.
  insert into public.projects (user_id, name, data)
  select user_id, 'Demo Living Room', $DEMO${
    "roomConfig": {
      "type": "Living Room", "shape": "rect",
      "w": 6, "h": 5, "wallH": 2.7, "wallT": 0.15,
      "floor": { "style": "wood", "color": "#c8a878" }

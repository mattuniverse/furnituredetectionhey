...
export async function runFurnitureDetectionWorkflow({
  ...
  if (!apiKey) {
    throw new RoboflowError(
      "ROBOFLOW_API_KEY is not set. Add it in Vercel.",
      { status: 500 }
    );
  }
  ...
  const workflowId =
    ...
    "furniture-vfurniture-q5tkw-hv0i3-1-yolo11n-t1-logic";
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new RoboflowError(
          `Roboflow returned ${res.status}: ${text}`,
          { status: res.status }
        );
      }
      ...
      const status =
        err instanceof RoboflowError ? err.status : null;

      const shouldRetry =
        attempt < retries &&
        (isAbort || !status || status >= 500);
  ...
  if (lastErr instanceof RoboflowError) {
    throw lastErr;
  }
